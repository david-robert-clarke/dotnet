...
public class Bank
{
   ...
   * @throws AccountExistsException when an account with the given number already exists. 
	   ...
	   */
	   do
	   {
		  ...
		  */
		  ...
		  {
			  AccountExistsException e = new AccountExistsException("" + a.getAccountNumber() + "");
			  throw e;  
		  }
		  i++;
	   }while(i < accounts.length);
	   
	   // if there are no avialable elements in which to store an account throw an exception
	   if (freeCell == false) 
	   {
	   ...
	   // else if there are available cells, place the account in the last avialable cell
   ...
   * @throws NotFoundException when there is no account with the specified number.
   * The associated message will be the requested account number as a String.
   **/
   public Account getAccount(int accountNumber, int pinNumber) throws NotFoundException
   {
	   int i = 0;
	   // find the account with this accountNumber and pinNumber
	   do
	   {
