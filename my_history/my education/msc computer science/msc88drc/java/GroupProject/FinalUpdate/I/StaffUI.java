...
public class StaffUI extends JFrame
{
  ...
  private JPanel salesSummaryPanel;  private JPanel lowerPanel;
  ...
  public StaffUI(String staffID, String firstName, String posit)
  {
    ...
    editProductButton = new JButton("Edit Details");
    ...
    replacePanel.add(new JScrollPane(stockList));
    tabbedPane.addTab("STOCK",  replacePanel);
    

    //Customer Tab
    viewCustomerButton = new JButton("View Details");
    viewCustomerButton.addActionListener(new CustomerListener());
    ...
    Object[] customerData = new Object[300];
    ...
    newPanel.add(new JScrollPane(customerList));
    ...
    Object[] supplierData = new Object[300];//could be more, hopefully less
    ...
    supplierAreaPanel.add(new JScrollPane(supplierList));
    ...
    Object[] staffData = new Object[300];//watch out, array sizes need change
    ...
    staffAreaPanel.add(new JScrollPane(staffList));
    ...
    staffOrderAreaPanel.add(new JScrollPane(staffOrderList));
  ...
  public class StockListener implements ActionListener
  {
	...
	if (stockList.getSelectedIndex() != -1) //doesnot equal no selection
	{
	  ...
	  RemoveStockUI removeStockUI = new RemoveStockUI(cdAttributes);
	  removeStockUI.setTitle("Remove CD");
	  removeStockUI.setSize(250,350);
	  removeStockUI.setResizable(false);
	  removeStockUI.setLocation(650,350);
	  removeStockUI.show();
	  removeStockUI.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
  ...
  public class CustomerListener implements ActionListener
  {
    public void actionPerformed(ActionEvent e)
    {
      ...
      if(source == viewCustomerButton)
      {
	if (customerList.getSelectedIndex() != -1) //does not equal no selection
	{
	  // Selection
	  String customerIDLName =customerList.getSelectedValue().toString();
	  WordSplit thisSplit = new WordSplit(customerIDLName);
	  ArrayList customerAttributes = thisSplit.getWords(); //arrayList,
	  String customerID = (String)customerAttributes.get(0);
	  ViewCustomerUI viewCustUI 
	    = new ViewCustomerUI(customerID);
	  viewCustUI.setTitle("View Customer Details");
	  viewCustUI.setSize(250, 350);
	  viewCustUI.setResizable(false);
	  viewCustUI.setLocation(650, 300);
	  viewCustUI.show();
	  viewCustUI.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
  ...
  public class StaffListener implements ActionListener
  {
	...
	addStaffUI.setLocation(650,300);
      ...
      {
	String staffIDLName = staffList.getSelectedValue().toString();
	WordSplit thisSplit = new WordSplit(staffIDLName);
	ArrayList customerAttributes = thisSplit.getWords();
	String staffID = (String)customerAttributes.get(0);
	RemoveStaffUI removeStaffUI = new RemoveStaffUI(staffID);
	removeStaffUI.setTitle("Remove Staff");
	removeStaffUI.setSize(250,350);
	...
	if (staffList.getSelectedIndex() != -1) //does not equal no selection
	{
	  ...
	  ArrayList customerAttributes = thisSplit.getWords(); //arrayList,
	  String staffID = (String)customerAttributes.get(0);
	  EditStaffUI editStaffUI = new EditStaffUI(staffID);
